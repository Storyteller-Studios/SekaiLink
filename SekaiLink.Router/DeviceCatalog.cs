...
public static class DeviceCatalog
{
    ...
    public static IReadOnlyList<DeviceCatalogEntry> All { get; } = Array.AsReadOnly(
        RoseDeviceCatalog.All.Select(d =>
                new DeviceCatalogEntry(d.Id, d.DisplayName, "RoseElsa", d.ProtocolFamily.ToString(), true))
    ...
    {
        return All.FirstOrDefault(d => d.Id.Equals(id, StringComparison.OrdinalIgnoreCase));
    }

    public static bool IsOppo(DeviceRoute route)
    {
        return route.Definition is OppoDeviceDefinition;
    }

    public static bool PrefersClassic(string id)
    {
        return OppoDeviceCatalog.Find(id) != null;
    }

    public static bool PrefersClassic(DeviceRoute route)
    {
        return route.Definition is OppoDeviceDefinition;
    }

    public static DeviceProfile? CreateProfile(DeviceRoute route)
    {
        return route.Definition switch
        {
    ...
    public static DeviceRoute RefineByProductId(DeviceRoute route, string productId)
    {
        ...
        if (!exact.IsSupported)
            throw new NotSupportedException($"Product ID {productId} does not support this OPPO protocol.");
    ...
    public static DeviceRoute CreateRoute(string id, string address, string? transport = null)
    {
        ...
        if (rose != null)
        {
            ...
            var gatt = rose.GattConfiguration ??
                       throw new NotSupportedException($"{rose.DisplayName} has no GATT configuration.");
        ...
        if (oppo == null && OppoDeviceCatalog.TryResolve(new DeviceIdentity { Name = id }, out var byName))
            oppo = byName;
        if (oppo == null) throw new ArgumentException($"Unknown or ambiguous model '{id}'.", nameof(id));
        if (!oppo.IsSupported)
            throw new NotSupportedException($"{oppo.DisplayName} does not support this OPPO protocol.");
        if (transport != null && !transport.Equals("gatt", StringComparison.OrdinalIgnoreCase) &&
        ...
        var endpointOppo = ble
            ? new TransportEndpoint(TransportKind.BleGatt, address, OppoCommands.SppServiceUuid.ToString("D"),
                GattWrite, GattNotify)
