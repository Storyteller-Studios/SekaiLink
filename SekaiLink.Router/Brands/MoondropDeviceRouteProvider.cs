...
internal sealed class MoondropDeviceRouteProvider : IDeviceRouteProvider
{
    ...
    {
        return identity.ServiceIds.Any(value => Guid.TryParse(value, out var parsed) && parsed == service);
    }

    private static TransportEndpoint BleEndpoint(DeviceIdentity identity, TransportEndpoint discovered, Guid service,
        Guid write, Guid notify)
    {
        return new TransportEndpoint(TransportKind.BleGatt, identity.Address ?? discovered.Address,
