...
namespace SekaiLink.Protocols.Models
{
    ...
    {
        public NoiseControlCapabilities(SupportStatus status = SupportStatus.Unknown, IReadOnlyList<NoiseMode>? modes = null)
        { Status = status; Modes = modes ?? Array.Empty<NoiseMode>(); }
    ...
    public enum NoiseModeId { Unknown, Normal, NoiseCancellation, Transparency, WindReduction, AdaptiveNoiseCancellation }
    public sealed class NoiseMode
    {
        ...
        { Id = id; Name = name ?? id.ToString(); ProtocolValue = protocolValue ?? Array.Empty<byte>(); IsSelectable = selectable; }
    ...
    public enum EqualizerFilterType { Peak, LowShelf, HighShelf, LowPass, HighPass, Notch }
