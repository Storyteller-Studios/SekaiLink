...
public sealed class DeviceProfile
{
    ...
    public bool Matches(DeviceIdentity identity)
    {
        ...
        if (!string.IsNullOrEmpty(identity.Model) &&
            identity.Model.Equals(Id, StringComparison.OrdinalIgnoreCase)) return true;
        return !string.IsNullOrEmpty(identity.Name) &&
               identity.Name.IndexOf(Id, StringComparison.OrdinalIgnoreCase) >= 0;
    }
}