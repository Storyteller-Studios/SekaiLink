...
{
    public ProtocolSessionStateChangedEventArgs(ProtocolSessionState state)
    {
        State = state;
    }

    public ProtocolSessionState State { get; }
}
...
public interface IProtocolSession : IAsyncDisposable
{
    ...
    Task<OperationResult<TResponse>> ExecuteAsync<TResponse>(object request, Func<object, byte[]> encode,
        Func<byte[], TResponse?> decode, TimeSpan timeout, IProtocolMessageMatcher? matcher = null,
...
public sealed class ProtocolSession : IProtocolSession
{
    ...
    public ProtocolSession(IProtocolTransport transport, TransportEndpoint? endpoint = null)
    {
    ...
    public async Task ConnectAsync(CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            SetState(ProtocolSessionState.TransportReady);
            SetState(ProtocolSessionState.Initializing);
            SetState(ProtocolSessionState.Ready);
        }
        catch
        {
            SetState(ProtocolSessionState.Faulted);
            throw;
        }
    }
    ...
    public async Task<OperationResult<TResponse>> ExecuteAsync<TResponse>(object request, Func<object, byte[]> encode,
        Func<byte[], TResponse?> decode, TimeSpan timeout, IProtocolMessageMatcher? matcher = null,
        CancellationToken cancellationToken = default)
    {
        ThrowIfDisposed();
        if (State != ProtocolSessionState.Ready)
            return OperationResult<TResponse>.Failure(OperationErrorKind.Disconnected, "The session is not ready.");
        if (request == null || encode == null || decode == null)
            return OperationResult<TResponse>.Failure(OperationErrorKind.InvalidParameter);
            ...
            using (var timeoutCts = CancellationTokenSource.CreateLinkedTokenSource(cancellationToken))
            {
                ...
                try
                {
                    ...
                    if (generation != _generation)
                        return OperationResult<TResponse>.Failure(OperationErrorKind.Disconnected);
                    try
                    {
                        return OperationResult<TResponse>.Success(decode(response), ConfirmationLevel.Applied,
                            response);
                    }
                    catch
                    {
                        return OperationResult<TResponse>.Failure(OperationErrorKind.InvalidResponse,
                            "Response decoding failed.", rawResponse: response);
                    }
                }
                catch (OperationCanceledException) when (cancellationToken.IsCancellationRequested)
                {
                    return OperationResult<TResponse>.Cancelled();
                }
                catch (OperationCanceledException)
                {
                    return OperationResult<TResponse>.Failure(OperationErrorKind.Timeout);
                }
            }
        }
        catch (OperationCanceledException) when (cancellationToken.IsCancellationRequested)
        ...
        {
            return OperationResult<TResponse>.Failure(OperationErrorKind.TransportError, ex.Message);
        }
        finally
        {
    ...
    private void OnDataReceived(object? sender, TransportDataReceivedEventArgs e)
    {
        ...
        foreach (var pending in _pending.Values)
            if (pending.Matcher == null || pending.Matcher.IsMatch(e.Data, pending.Request))
                if (pending.Completion.TrySetResult(e.Data))
                    return;
    ...
    {
        if (e.State == TransportState.Disconnected) SetState(ProtocolSessionState.Disconnected);
    }

    private void SetState(ProtocolSessionState value)
    {
        State = value;
        StateChanged?.Invoke(this, new ProtocolSessionStateChangedEventArgs(value));
    }
