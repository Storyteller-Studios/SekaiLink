...
namespace SekaiLink.Protocols.Features
{
    ...
    public enum AudioCodecMode { AacSbc, Ldac, Lhdc, Lc3, Aptx, AptxHd, AptxAdaptive }
    public enum DeviceLanguage { Chinese, English }
    public enum SpatialAudioMode { Off, HeadTracking, Fixed, Music, Movie, Game, Television }
    ...
    public sealed class MicrophoneCommand
    {
        ...
        { Opcode = opcode; Arguments = arguments ?? Array.Empty<byte>(); }
        public byte Opcode { get; }
        public IReadOnlyList<byte> Arguments { get; }
    }
    public enum DeviceSide { Left, Right }
    public enum DeviceGesture { Single, Double, Triple, LongPress }
    public enum GestureAction { VolumeUp, VolumeDown, PreviousTrack, NextTrack, VoiceAssistant, PlayPause, None, NoiseControl }
    public enum FindDeviceAction { Stop, Left, Right }
