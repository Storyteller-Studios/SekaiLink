...
namespace SekaiLink.Rose.Devices.Devices.Auramic
{
    ...
    internal sealed class AuramicFeatureCommandMapper : IFeatureCommandMapper<RoseFrame>
    {
        ...
        public RoseFrame Encode(FeatureDefinition command)
        {
            ...
            if (!Opcodes.Contains(mic.Opcode)) throw new ArgumentOutOfRangeException(nameof(command), $"Unknown Auramic opcode 0x{mic.Opcode:X2}.");
            if (mic.Arguments.Count == 0) throw new ArgumentException("Auramic write commands require at least one argument.", nameof(command));
            return new RoseFrame(mic.Opcode, mic.Arguments[0], mic.Arguments.Skip(1).ToArray());
        }
        public string? GetResponseIdentifier(RoseFrame frame) => Opcodes.Contains(frame.Group) ? FeatureIdentifiers.MicrophoneControl : null;
    }
}
