...
internal sealed class RoseRsFeatureCommandMapper : IFeatureCommandMapper<RoseFrame>
{
    ...
    {
        if (family != RoseFamily.RsCommon && family != RoseFamily.RsCommonV2)
    ...
    public IReadOnlyList<FeatureDefinition> InitializationCommands { get; } =
        new[] { new FeatureDefinition(FeatureIdentifiers.DeviceInformation) };
    ...
    public RoseFrame Encode(FeatureDefinition command)
    {
        ...
        return command.Identifier switch
        {
            ...
            FeatureIdentifiers.WearDetection => Flag(RoseDeviceFeatures.WearDetection, 0x08, Require<bool>(command),
                true),
            FeatureIdentifiers.TouchControls => Flag(RoseDeviceFeatures.TouchControls, 0x07, Require<bool>(command),
                true),
    ...
    {
        return frame.Group switch
        {
            0x09 => FeatureIdentifiers.NoiseControl,
            0x0E => FeatureIdentifiers.LowLatency,
            0x2A or 0x3E => FeatureIdentifiers.Equalizer,
            // RsCommon also uses 0x2B for multipoint. Its response cannot be
            // identified by group alone, so leave the label unknown.
            0x2B => _family == RoseFamily.RsCommonV2 ? FeatureIdentifiers.AudioCodec : null,
    ...
    private RoseFrame Read(string id)
    {
        if (id is FeatureIdentifiers.DeviceInformation or FeatureIdentifiers.Battery)
            return _family == RoseFamily.RsCommon
                ? new RoseFrame(0xFA, 0x01,
                    new byte[] { 0x07, 0x08, 0x09, 0x0C, 0x0D, 0x0E, 0x12, 0x2A, 0x2B, 0x2C, 0x2D, 0x2E, 0x33 })
                : new RoseFrame(0xFA, 0x01,
                    new byte[]
                    {
                        0x07, 0x08, 0x09, 0x0C, 0x0D, 0x0E, 0x12, 0x2A, 0x2B, 0x2C, 0x2D, 0x2E, 0x2F, 0x31, 0x32, 0x33,
                        0x36, 0x37, 0x38, 0x39, 0x3A, 0x3B, 0x3C, 0x3D, 0x3F, 0x45, 0x46, 0x49, 0xFF
                    });
    ...
    private RoseFrame Noise(NoiseModeId mode)
    {
        ...
        return Frame(0x09,
            mode switch
            {
                NoiseModeId.NoiseCancellation => 1, NoiseModeId.Normal => 2, NoiseModeId.Transparency => 3,
                NoiseModeId.WindReduction => 4, _ => throw new ArgumentOutOfRangeException(nameof(mode))
            });
    }
    ...
    private RoseFrame Audio(AudioCodecMode mode)
    {
        ...
        return Frame(0x2B,
            mode switch
            {
                AudioCodecMode.AacSbc => 0, AudioCodecMode.Ldac => 1, AudioCodecMode.Lhdc => 2,
                AudioCodecMode.Lc3 => 3, AudioCodecMode.Aptx => 4, AudioCodecMode.AptxHd => 5,
                AudioCodecMode.AptxAdaptive => 6, _ => throw new ArgumentOutOfRangeException(nameof(mode))
            });
    }
    ...
    private RoseFrame Gesture(GestureMapping value)
    {
        ...
        var key = (byte)((value.Side == DeviceSide.Left ? 0 : 0x10) + value.Gesture switch
        {
            DeviceGesture.Single => 1, DeviceGesture.Double => 2, DeviceGesture.Triple => 3,
            DeviceGesture.LongPress => 4, _ => throw new ArgumentOutOfRangeException()
        });
    ...
    private RoseFrame Find(FindDeviceAction value)
    {
        ...
        return Frame(0x2F,
            value switch
            {
                FindDeviceAction.Left => 1, FindDeviceAction.Right => 2, FindDeviceAction.Stop => 4,
    ...
    private RoseFrame Spatial(SpatialAudioMode value)
    {
        ...
        return value switch
        {
            SpatialAudioMode.Off => Frame(0x33, 0), SpatialAudioMode.HeadTracking => Frame(0x38, 1),
            SpatialAudioMode.Fixed => Frame(0x38, 2), _ => throw new ArgumentOutOfRangeException(nameof(value))
        };
    }
    ...
    private RoseFrame Flag(RoseDeviceFeatures feature, byte group, bool enabled, bool inverted)
    {
        Need(feature, group.ToString("X2"));
        return Frame(group, enabled ^ inverted ? 1 : 0);
    }

    private RoseFrame Dynamic(RoseDeviceFeatures feature, byte group, byte value)
    {
        Need(feature, group.ToString("X2"));
        return Frame(group, value);
    }

    private static RoseFrame Frame(int group, int command)
    {
        return new RoseFrame((byte)group, (byte)command, Array.Empty<byte>());
    }

    private void Need(RoseDeviceFeatures feature, string id)
    {
        if ((_features & feature) == 0) throw Unsupported(id);
    }
    ...
    {
        var v = Require<int>(c);
        if (v < 1 || v > 255) throw new ArgumentOutOfRangeException(nameof(c));
        return (byte)(v - 1);
    }
    ...
    {
        var v = Require<int>(c);
        if (v < 0 || v > 255) throw new ArgumentOutOfRangeException(nameof(c));
        return (byte)v;
    }

    private static T Require<T>(FeatureDefinition c)
    {
        return c.Value is T v
            ? v
            : throw new ArgumentException($"Feature '{c.Identifier}' expects {typeof(T).Name}.", nameof(c));
    }
}