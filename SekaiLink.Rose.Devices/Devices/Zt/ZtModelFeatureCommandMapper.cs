...
internal sealed class ZtModelFeatureCommandMapper : IFeatureCommandMapper<RoseFrame>
{
    ...
    public ZtModelFeatureCommandMapper(RoseDeviceType type, RoseDeviceFeatures features)
    {
        _type = type;
        _features = features;
        var commands = new List<FeatureDefinition>
            { Read(FeatureIdentifiers.DeviceInformation), Read(FeatureIdentifiers.FirmwareVersion) };
    ...
    public RoseFrame Encode(FeatureDefinition c)
    {
        ...
        if (c.Value == null) return EncodeRead(c.Identifier);
    ...
    public string? GetResponseIdentifier(RoseFrame f)
    {
        return (f.Group, f.Command) switch
        {
    ...
    private RoseFrame EncodeRead(string id)
    {
        return id switch
        {
    ...
    private RoseFrame Noise(NoiseModeId mode)
    {
        ...
        if (_type == RoseDeviceType.EarfreeI3)
            return mode switch
            {
    ...
    private RoseFrame Eq(object value)
    {
        ...
        {
            "pop" => 0, "hifi" => 1, "rock" => 2, "light" when _type != RoseDeviceType.EarfreeI3 => 3,
            _ => throw new ArgumentOutOfRangeException(nameof(value))
        };
        return Frame(2, 0x81, (byte)v);
    }
    ...
    private RoseFrame Audio(AudioCodecMode mode)
    {
        ...
        return mode switch
        {
            AudioCodecMode.AacSbc => Frame(8, 0x81, 0), AudioCodecMode.Ldac => Frame(8, 0x81, 1),
            AudioCodecMode.Lhdc when _type == RoseDeviceType.Headfree => Frame(8, 0x81, 2),
            _ => throw new ArgumentOutOfRangeException(nameof(mode))
        };
    }

    private RoseFrame Game(bool enabled)
    {
        Need(RoseDeviceFeatures.LowLatency, FeatureIdentifiers.LowLatency);
        return Frame(6, 0x83, enabled ? (byte)0 : (byte)1);
    }

    private RoseFrame Multipoint(bool enabled)
    {
        Need(RoseDeviceFeatures.Multipoint, FeatureIdentifiers.Multipoint);
        return Frame(8, 0x81, enabled ? (byte)0 : (byte)1);
    }

    private RoseFrame Touch(bool enabled)
    {
        Need(RoseDeviceFeatures.TouchControls, FeatureIdentifiers.TouchControls);
        return Frame(10, 0x81, enabled ? (byte)0 : (byte)1);
    }

    private RoseFrame LevelCommand(int command, FeatureDefinition c)
    {
        Need(RoseDeviceFeatures.NoiseLevel, c.Identifier);
        return Frame(6, command, NoiseLevel(c));
    }

    private RoseFrame Prompt(FeatureDefinition c)
    {
        Need(RoseDeviceFeatures.PromptToneLevel, FeatureIdentifiers.PromptToneLevel);
        return Frame(14, _type == RoseDeviceType.Headfree ? 3 : 0x83, Level(c));
    }

    private RoseFrame Language(DeviceLanguage language)
    {
        Need(RoseDeviceFeatures.Language, FeatureIdentifiers.DeviceLanguage);
        return Frame(15, 0x81, language == DeviceLanguage.English ? (byte)1 : (byte)0);
    }

    private RoseFrame Gain(FeatureDefinition c)
    {
        Need(RoseDeviceFeatures.NoiseLevel, FeatureIdentifiers.GainLevel);
        return Frame(2, 0x83, Byte(c));
    }

    private RoseFrame Gesture(GestureMapping v)
    {
        Need(RoseDeviceFeatures.TouchControls, FeatureIdentifiers.GestureMappings);
        return Frame(4, 0x81, (byte)v.Side, (byte)v.Gesture, (byte)v.Action);
    }

    private RoseFrame Find(FindDeviceAction v)
    {
        Need(RoseDeviceFeatures.FindDevice, FeatureIdentifiers.FindDevice);
        var command = _type == RoseDeviceType.Headfree ? (byte)0x02 : (byte)0x82;
        return v switch
        {
            FindDeviceAction.Left => Frame(14, command, 0, 1), FindDeviceAction.Right => Frame(14, command, 1, 1),
            FindDeviceAction.Stop => Frame(14, command, 3, 0), _ => throw new ArgumentOutOfRangeException(nameof(v))
        };
    }

    private RoseFrame Query(RoseDeviceFeatures feature, int group, int command)
    {
        Need(feature, feature.ToString());
        return Frame(group, command);
    }

    private void Need(RoseDeviceFeatures f, string id)
    {
        if ((_features & f) == 0) throw new NotSupportedException($"{_type} does not advertise '{id}'.");
    }

    private static RoseFrame Frame(int g, int c, params byte[] p)
    {
        return new RoseFrame((byte)g, (byte)c, p);
    }

    private static FeatureDefinition Read(string id)
    {
        return new FeatureDefinition(id);
    }

    private void Add(ICollection<FeatureDefinition> list, RoseDeviceFeatures feature, string id)
    {
        if ((_features & feature) != 0) list.Add(Read(id));
    }

    private static byte Level(FeatureDefinition c)
    {
        var v = Require<int>(c);
        if (v < 1 || v > 5) throw new ArgumentOutOfRangeException();
        return (byte)(v - 1);
    }

    private static byte NoiseLevel(FeatureDefinition c)
    {
        return Require<int>(c) switch { 1 => 0, 3 => 1, 5 => 2, _ => throw new ArgumentOutOfRangeException() };
    }
