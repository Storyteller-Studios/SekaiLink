...
namespace SekaiLink.Oppo.Devices
{
    ...
    {
        private static readonly IReadOnlyList<OppoDeviceDefinition> Definitions = Array.AsReadOnly(CreateDefinitions());
        private static readonly IReadOnlyDictionary<string, OppoDeviceDefinition> ById =
            Definitions.ToDictionary(device => device.Id, StringComparer.OrdinalIgnoreCase);

        public static IReadOnlyList<OppoDeviceDefinition> All => Definitions;
        ...
        public static OppoDeviceDefinition? FindFromProductIdResponse(OppoFrame frame) =>
            OppoCommands.TryParseProductId(frame, out var productId) ? Find(productId) : null;
        ...
        public static bool TryResolve(DeviceIdentity identity, out OppoDeviceDefinition? definition)
        {
            ...
            foreach (var candidate in Definitions)
            {
                ...
                var score = model == known || name == known ? 100 :
                    model.StartsWith(known, StringComparison.Ordinal) || name.StartsWith(known, StringComparison.Ordinal) ? 80 : 0;
        ...
        private static OppoDeviceDefinition[] CreateDefinitions() => CreateGeneratedDefinitions();
