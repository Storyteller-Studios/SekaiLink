...
namespace SekaiLink.Oppo.Devices
{
    ...
    public sealed class OppoNoiseOption
    {
        ...
        { Key = key; ProtocolIndex = protocolIndex; IsSelectable = selectable; }
    ...
    public sealed class OppoEqualizerOption
    {
        ...
        { ProtocolIndex = protocolIndex; ModeType = modeType; }
        public byte ProtocolIndex { get; }
    ...
    public sealed class OppoDeviceDefinition
    {
        ...
        public bool Supports(OppoDeviceFeatures feature) => (Features & feature) == feature;
        ...
        public DeviceProfile CreateProfile(TransportEndpoint endpoint)
        {
            ...
            {
                new CapabilityDescriptor(FeatureIdentifiers.Battery, SupportStatus.Supported, true, false, true)
                    { Unit = "percent", Minimum = 0, Maximum = 100, Step = 1 },
                new CapabilityDescriptor(FeatureIdentifiers.DeviceInformation, SupportStatus.Supported, true)
            };
            ...
            if (Supports(OppoDeviceFeatures.NoiseControl))
            {
                ...
                foreach (var option in NoiseOptions)
                    if (option.IsSelectable && !options.Contains(option.Key)) options.Add(option.Key);
                capabilities.Add(new CapabilityDescriptor(FeatureIdentifiers.NoiseControl, SupportStatus.Supported, true, true, true)
            ...
            if (Supports(OppoDeviceFeatures.Equalizer))
            {
                ...
                capabilities.Add(new CapabilityDescriptor(FeatureIdentifiers.Equalizer, SupportStatus.Supported, true, true, true)
                    { Options = options.AsReadOnly() });
            }
            if (Supports(OppoDeviceFeatures.LowLatency))
            {
                types.Add(typeof(ILowLatencyFeature));
                capabilities.Add(new CapabilityDescriptor(FeatureIdentifiers.LowLatency, SupportStatus.Supported, true, true, true)
                    { Options = new[] { "off", "on" } });
            }
            if (Supports(OppoDeviceFeatures.WearDetection))
            {
                types.Add(typeof(IWearDetectionFeature));
                capabilities.Add(new CapabilityDescriptor(FeatureIdentifiers.WearDetection, SupportStatus.Supported, true, true, true));
            }
            if (Supports(OppoDeviceFeatures.SpatialAudio))
            {
                types.Add(typeof(ISpatialAudioFeature));
                capabilities.Add(new CapabilityDescriptor(FeatureIdentifiers.SpatialAudio, SupportStatus.Supported, true, true, true)
                    { Options = SpatialTypes.Contains((byte)2) ? new[] { "off", "fixed", "track" } : new[] { "off", "fixed" } });
            }
            ...
            return new DeviceProfile(Id, endpoint.Kind == TransportKind.BleGatt ? ProtocolFamily.OppoGatt : ProtocolFamily.OppoSpp, endpoint)
