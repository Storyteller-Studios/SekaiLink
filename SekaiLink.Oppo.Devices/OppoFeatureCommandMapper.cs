...
internal sealed class OppoFeatureCommandMapper : IFeatureCommandMapper<OppoFrame>
{
    ...
    public OppoFeatureCommandMapper(OppoDeviceDefinition device)
    {
        _device = device;
    }
    ...
    public string? GetResponseIdentifier(OppoFrame frame)
    {
    ...
    private OppoFrame Read(string id)
    {
        return id switch
        {
            ...
            FeatureIdentifiers.NoiseControl when _device.Supports(OppoDeviceFeatures.NoiseControl) => OppoCommands
                .QueryNoiseMode(),
            FeatureIdentifiers.Equalizer when _device.Supports(OppoDeviceFeatures.Equalizer) => new OppoFrame(
                OppoCommands.QueryEqualizer, Array.Empty<byte>()),
            FeatureIdentifiers.SpatialAudio when _device.Supports(OppoDeviceFeatures.SpatialAudio) => new OppoFrame(
                0x012A, Array.Empty<byte>()),
            FeatureIdentifiers.LowLatency when _device.Supports(OppoDeviceFeatures.LowLatency) => QueryFeature(
                GameFeature),
            FeatureIdentifiers.WearDetection when _device.Supports(OppoDeviceFeatures.WearDetection) =>
                QueryFeature(0x04),
            _ => throw Unsupported(id)
        };
    }

    private OppoFrame Write(FeatureDefinition command)
    {
        return command.Identifier switch
        {
            FeatureIdentifiers.NoiseControl when _device.Supports(OppoDeviceFeatures.NoiseControl) => SetNoise(
                command.Value!),
            FeatureIdentifiers.Equalizer when _device.Supports(OppoDeviceFeatures.Equalizer) => SetEqualizer(
                command.Value!),
            FeatureIdentifiers.LowLatency when _device.Supports(OppoDeviceFeatures.LowLatency) => SetFeature(
                GameFeature, Require<bool>(command)),
            FeatureIdentifiers.WearDetection when _device.Supports(OppoDeviceFeatures.WearDetection) => SetFeature(0x04,
                Require<bool>(command)),
            FeatureIdentifiers.SpatialAudio when _device.Supports(OppoDeviceFeatures.SpatialAudio) => SetSpatial(
                command.Value!),
            _ => throw Unsupported(command.Identifier)
        };
    }

    private static OppoFrame QueryFeature(byte feature)
    {
        return new OppoFrame(0x010D, new[] { (byte)1, feature });
    }

    private static OppoFrame SetFeature(byte feature, bool enabled)
    {
        return OppoCommands.SetFeatureValue(feature, enabled);
    }
    ...
    private OppoFrame SetNoise(object value)
    {
        ...
        if (value is NoiseModeId mode)
            ...
            key = value is OppoNoiseOption option
                ? option.Key
                : value as string ?? throw new ArgumentException("Expected a noise mode.");
    ...
    {
        return command.Value is T value
            ? value
            : throw new ArgumentException($"Feature '{command.Identifier}' expects {typeof(T).Name}.");
    }

    private static NotSupportedException Unsupported(string id)
    {
        return new NotSupportedException($"No verified OPPO mapping for '{id}'.");
    }
}